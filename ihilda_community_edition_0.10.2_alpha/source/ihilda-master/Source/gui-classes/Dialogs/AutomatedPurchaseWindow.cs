				...
				base (Gtk.WindowType.Toplevel)
		{
			...
			backgroundWorker.DoWork += (object sender, DoWorkEventArgs e) => {
				...
				while (!backgroundWorker.CancellationPending) {
					...
					Task<Response <BookOfferResult>> task = BookOffers.GetResult (automatedOrder.TakerGets, automatedOrder.TakerPays, ni);
					if (task == null) {
						// TODO error handling
					...
					for (int i = 0; i < offers.Length; i++) {
						...
						bestPrice = offer.taker_pays.GetNativeAdjustedCostAt (offer.TakerGets);
						...
						if (paysSum < minPaysAmount) {
							...
							tooSmallMessage.AppendLine (minPaysAmount.ToString ());
						...
						/*
						Tuple <bool, IEnumerable<RippleOfferTransaction>> tuple = OrderSubmitter.SubmitOrders (new AutomatedOrder [] { ao }, rw, ni);
